...
public:
	Solver(SolverConfiguration config = {}): config_(config), comp_manager_(config_,
...
private:
    ...
    /* The decision level after all assumptions have been applied. During the search,
     * no backtracking beyond this level is allowed */
    int decision_level_after_assumptions_ = 0;

    // states for each variable whether it occurs in an added clause
    vector<bool> variables_in_added_clauses_;
    ...
    // corrects changes to the clause offsets in added_long_clauses_
    void translate();

    // called before preprocessing
    void earlyPrepare();

    bool applyAssumptions();

    // called after preprocessing and assumptions
    void latePrepare();

    // called after solving
    void cleanup();

    // called either in latePrepare or decideLiteral, depending on the mode
    void updateVariablesInAddedClauses();
    ...
    void decayScoresOf (Component & comp) {
	   for (auto it = comp.varsBegin(); *it != varsSENTINEL; it++) {
	          literal(LiteralID(*it,true)).activity_score_ *=0.5;
	          literal(LiteralID(*it,false)).activity_score_ *=0.5;
	          literal(LiteralID(*it,true)).prevalence_score_ *=0.5;
	          literal(LiteralID(*it,false)).prevalence_score_ *=0.5;
	       }
	}
	...
	float scoreOf(VariableIndex v) {
		...
		score += 10.0 * literal(LiteralID(v, true)).totalScore();
		score += 10.0 * literal(LiteralID(v, false)).totalScore();
...
//		score += (10*stack_.get_decision_level()) * literal(LiteralID(v, false)).activity_score_;
	...
	void setConflictState(LiteralID litA, LiteralID litB) {
        ...
        violated_clause_ofs = NOT_A_CLAUSE;
	}
	...
	void initStack(unsigned int resSize) {
        ...
        // resets the assignments
        literal_values_.clear();
        literal_values_.resize(resSize + 1, X_TRI);
    ...
    vector<ClauseImplicationLinks> uip_clause_implication_links_;

...
template <class T>
 class BitUnstuffer {
 public:
  BitUnstuffer(T *data): p(data){}

  unsigned int unstuff(const unsigned int num_bits_val){
      // the expression after the & generates a mask with exactly num_bits_val 1s at the end
      unsigned int result = ((*p) >> end_of_bits_) & ((1 << num_bits_val) - 1);
      end_of_bits_ += num_bits_val;

      // the value might be spread over two entries of data
      if (end_of_bits_ > _bits_per_block){
        end_of_bits_ -= _bits_per_block;
        //again, a mask ending in 1s, this time using end_of_bits_, as that currently captures the number of remaing bits in the value
        result |= ((*(++p)) & ((1 << end_of_bits_) - 1)) << (num_bits_val - end_of_bits_);
      }
      else if (end_of_bits_ == _bits_per_block){
        end_of_bits_ -= _bits_per_block;
        p++;
 ...
 private:
  T *p = nullptr;
  // in the current block
  // the bit postion just after the last bit read
  unsigned end_of_bits_ = 0;
...
public:
  static unsigned bits_per_variable(bool use_previous = false) {
    return use_previous ? _previous_bits_per_variable : _bits_per_variable;
  }
  static unsigned variable_mask() {
      return _variable_mask;
  }
  static unsigned bits_per_clause(bool use_previous = false) {
    return use_previous ? _previous_bits_per_clause : _bits_per_clause;
  }
  ...
  static unsigned bits_of_data_size(bool use_previous = false){
    return use_previous ? _previous_bits_of_data_size : _bits_of_data_size;
  }

  // will be false if adjustment would not change anything
  static bool testPackSizeAdjustment(unsigned int maxVarId, unsigned int maxClId);
  ...
  void set_model_count(const mpz_class &rn, unsigned time) {
    model_count_ = rn;
    length_solution_period_and_flags_ = (time - creation_time_) << 1 | (length_solution_period_and_flags_ & 1);
  }
...
protected:
  ...
  // stores the previous sizes for adjustng the existing entries
  static unsigned int _previous_bits_per_clause , _previous_bits_per_variable, _previous_bits_of_data_size;
};

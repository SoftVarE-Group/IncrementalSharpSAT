...
public:
  ...
  // if the number of variables or clauses has changed, this will repack the entries with the new data sizes
  void adjustPackSize(unsigned int maxVarId, unsigned int maxClId);

  void setSuperComponent(Component &super_comp);

  // applies changes to the clause ids to the cache
  void translate(vector<unsigned int>& translation_table);

  // removes every entry no longer valid becvaus of the addition of damaging_clause
  void remove_damaged_entries(vector<LiteralID>::iterator damaging_clause);
  ...
  bool cache_full(){
    return statistics_.cache_bytes_memory_usage() >= config_.maximum_cache_size_bytes_;
  }

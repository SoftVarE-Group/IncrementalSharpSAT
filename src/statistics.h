...
public:
  ...
  double time_for_preparations_ = 0.0;
  ...
  unsigned long num_conflict_unit_clauses_ = 0;

  unsigned long times_conflict_clauses_cleaned_ = 0;

  /// number of all decisions made. Also used as internal time, so it should not be reset between calls
  unsigned long num_total_decisions_ = 0;
  unsigned long num_decisions_at_start_ = 0;
  ...
  // number of all conflicts occurred. Also used as internal time, so it should not be reset between calls
  unsigned long num_total_conflicts_ = 0;
  unsigned long num_conflicts_at_start_ = 0;

  // number of clauses overall learned
  unsigned num_total_clauses_learned_ = 0;

  // reuse statistics
  unsigned long num_reused_long_clauses_ = 0;
  unsigned long num_reused_binary_clauses_ = 0;
  unsigned long num_reused_unit_clauses_ = 0;
  uint64_t num_reused_components_ = 0;
  ...
  void prepare(){
    num_decisions_at_start_ = num_total_decisions_;
    num_failed_literal_tests_ = 0;
    num_failed_literals_detected_ = 0;
    num_conflicts_at_start_ = num_total_conflicts_;

    num_cache_hits_ = 0;
    num_cache_look_ups_ = 0;
    sum_cache_hit_sizes_ = 0;
  }

  void store_reuse_data(){
    num_reused_long_clauses_ = num_long_conflict_clauses_;
    num_reused_binary_clauses_ = num_binary_conflict_clauses_;
    num_reused_unit_clauses_ = num_conflict_unit_clauses_;

    // -1 because the super component will have been replaced with a new one
    num_reused_components_ = num_cached_components_ - 1;
  }
  ...
  unsigned long num_conflict_clauses() {
    return num_long_conflict_clauses_ + num_binary_conflict_clauses_ + num_conflict_unit_clauses_;
  }
  unsigned long num_reused_clauses() {
    return num_reused_long_clauses_ + num_reused_binary_clauses_ + num_reused_unit_clauses_;
  }
  ...
  void incorporateClauseRemoval(const vector<LiteralID> &clause) {
    if (clause.size() == 1)
      num_unit_clauses_--;
    else if (clause.size() == 2)
      num_binary_clauses_--;
